...
app.post( "/scenarios", function( req, res, next ) {
  ...
  var scenarioFilename =
    "documents/ITDG/" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) + "/saveState.vwf.json";
  var scenarioFilename2 =
    "documents/ITDG/" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) + "/saveState_" + now.valueOf() + ".vwf.json";

  mkdirp( "documents/ITDG/" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) ).
    ...
    then( () => res.status( 201 ).send( { document: { uri: helpers.JoinPath( "/ITDG", helpers.GenerateInstanceID(), "load", req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) ) } } ) ).
...
app.post( "/sessions", function( req, res, next ) {
  ...
  var sessionFilename =
    "documents/ITDG/" + "class_" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) +
  ...
  var sessionFilename2 =
    "documents/ITDG/" + "class_" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) +
      ...
      "_Unit" + req.body.unit +
      "_" + now.getFullYear() +
      "/saveState_" + now.valueOf() + ".vwf.json";

  mkdirp( "documents/ITDG/" + "class_" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) + "_Co" + req.body.company + "_Plt" + req.body.platoon + "_Unit" + req.body.unit + "_" + now.getFullYear() ).
    ...
    then( () => res.status( 201 ).send( { document: { uri: helpers.JoinPath( "/ITDG", helpers.GenerateInstanceID(), "load", "class_" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) + "_Co" + req.body.company + "_Plt" + req.body.platoon + "_Unit" + req.body.unit + "_" + now.getFullYear() ) } } ) ).
    catch( error => next( error ) );
