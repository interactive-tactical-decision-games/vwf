...
app.get( "/phase-list", function( req, res ) {
  var phaseDir = "documents/ITDG/tracking/phase";

  // Ensure that the metrics directory exists, then get the list of files in it
  mkdirp( phaseDir )
    .then( made => fs.readdirAsync( phaseDir ) )
    .then( files => res.send( { phaseFilenames: files || [] } ) )
    .catch( err => next( err ) );
} );
