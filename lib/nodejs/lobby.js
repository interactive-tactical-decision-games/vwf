...
app.get( "/overlay-list", function( req, res ) {
  var overlayDir = "documents/ITDG/overlay";

  // Ensure that the overlay directory exists, then get the list of files in the image directory
  mkdirp( overlayDir )
    .then( made => fs.readdirAsync( overlayDir ) )
    .then( files => res.send( { overlayFilenames: files || [] } ) )
    .catch( err => next( err ) );
} );
...
app.post( "/sessions", function( req, res, next ) {
  ...
  var sessionUnit =
    req.body.unit.trim().replace( /[^0-9A-Za-z]+/g, "-" );
...
function zipFolder( stagingDir, zipFilePath ) {
  ...
  return new Promise( function( resolve, reject ) {
    ...
    // Pipe the zipper to the output stream.

    archive.pipe( output );

    // Write the directory to the zipper.
