...
app.post( "/sim", function( req, res, next ) {
  var simDirectory = "sim";
  var data = req.body;
  var filePath = simDirectory + "/" + data.filePathFromSim;
  fs.writeFile( filePath, data.simData, function( err ) {
    if ( err ) {
      console.error( err );
      return;
    }
    execFile( data.simExe, [
      data.commandLineFlag,
      data.filePathFromSim
    ], {
      cwd: simDirectory
    }, function( err ) {
      if ( err ) {
