...
app.post( "/scenarios", function( req, res, next ) {
  ...
  mkdirp( "documents/ITDG/" + scenarioName ).
    ...
    then( () => res.status( 201 ).send( { document: { uri: helpers.JoinPath( "/ITDG/index.vwf", "load", scenarioName ) } } ) ).
...
app.post( "/sessions", function( req, res, next ) {
  ...
  mkdirp( "documents/ITDG/" + sessionName ).
    ...
    then( () => res.status( 201 ).send( { document: { uri: helpers.JoinPath( "/ITDG/index.vwf", "load", sessionName ) } } ) ).
    catch( error => next( error ) );
