...
function Serve( request, response, parsedRequest ) {
    ...
    // Test if the sandtable functionality can serve this request.
    if ( sandtable.Serve( request, response, parsedRequest ) ) {
        return true;
    }
...
function IsPublicAPI( request, parsedRequest ) {
    return sandtable.IsPublicAPI( request, parsedRequest );
}
