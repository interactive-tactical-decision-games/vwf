    ...
    if ( segments.length >= 2 ) {
        ...
        form.on('fileBegin', function(name, file) {
            ...
            if ( filetype != 'wmv' ) {
                ...
                convertedPath = helpers.JoinPath( './documents', relPath, newName);
            }
        ...
        form.parse(request, function(err, fields, files) {
            if ( ! err ) {
                if ( !isReady() ) {
                    ...
                    response.end('Conversion Worker Not Yet Ready!');
                }
                else if ( files[0] ) {
                   var videoData = new Uint8Array(files[0]);
                   runCommand('-i ' + file.path + ' -vf showinfo -strict -2 ' + convertedPath + '', file.path, videoData);
