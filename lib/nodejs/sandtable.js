...
const { curryRightN, findLast: find, flow, get, head, isEqual, last, omit } = require('lodash/fp');
const parseUrl = curryRightN(2)(url.parse)(true);
const getParams = flow(parseUrl, get('query.params'), JSON.parse, omit('requestID'));

function OnOverlayRequest( requestObj ) {
    let addToQueue = true;
    if ( requestObj.segments[ 1 ] === "GenerateOverlay" ) {
        const params = getParams(requestObj.request.url);
        const cachedData = flow(find(flow(head, isEqual(params))), last)(requestCache);
        const hasCached = cachedData !== undefined;

        if (hasCached) {
            requestObj.response.json(cachedData);
        } else {
            requestObj.onData = function (data) {
                const result = JSON.parse(data);
                const success = result.status === 1;
                if (success) {
                    requestCache.push([params, result]);
                }
            }
        }

        addToQueue = !hasCached;
    }
    if (addToQueue) {
        addToQueueAndCheckStatus( requestObj );
    }
