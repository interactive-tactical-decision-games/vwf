...
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { event: "connection", id: socket.id } }, json: true } );
...
var messageForLog, idForLog;
    ...
    if ( Object.keys( global.instances[ namespace ].clients ).length === 1 ) {
        ...
        if ( saveObject ) {
            socket.emit( 'message', messageForLog = {
                action: "setState",
                parameters: [ saveObject ],
                time: global.instances[ namespace ].getTime( )
            } );
idForLog = socket.id;
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { method: "message", message: messageForLog, id: idForLog } }, json: true } );  // console.log( socket.id, "setState" );
        }
        else {
...
idForLog = socket.id;
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { method: "message", message: messageForLog, id: idForLog } }, json: true } );  // console.log( socket.id, "createNode", "http://vwf.example.com/clients.vwf" );
...
idForLog = socket.id;
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { method: "message", message: messageForLog, id: idForLog } }, json: true } );  // console.log( socket.id, "createNode", ( processedURL.public_path === "/" ? "" : processedURL.public_path ) + "/" + processedURL.application );
        }
        ...
        if ( ! global.instances[ namespace ].pendingList.pending ) {
            ...
            firstclient.emit( 'message', messageForLog = {
                action: "getState",
                respond: true,
                time: global.instances[ namespace ].getTime( )
            } );
idForLog = firstclient.id;
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { method: "message", message: messageForLog, id: idForLog } }, json: true } );  // console.log( firstclient.id, "getState" );
    ...
    for ( var i in global.instances[ namespace ].clients ) {
        var client = global.instances[ namespace ].clients[ i ];
        if ( !client.pending ) {
            client.emit ( 'message',  messageForLog = clientNodeMessage );
idForLog = client.id;
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { method: "message", message: messageForLog, id: idForLog } }, json: true } );  // console.log( client.id, clientNodeMessage.action );
        }
    }
    if ( global.instances[ namespace ].pendingList.pending ) {
...
if ( message.action ) {
    rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { event: "message", message: message, id: socket.id } }, json: true } );
}
        ...
        if ( message.result === undefined ) {
            ...
            for ( var i in global.instances[ namespace ].clients ) {
                ...
                if ( ! client.pending ) {
                    client.emit( 'message', messageForLog = message );
idForLog = client.id;
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { method: "message", message: messageForLog, id: idForLog } }, json: true } );  // console.log( client.id, "pending", "setState" );
                }
            }
            ...
            for ( var i in global.instances[ namespace ].clients ) {
                ...
                if ( client.pending ) {
                    ...
                    client.emit( 'message', messageForLog = { action: "setState", parameters: [ state ], time: setStateTime } );
idForLog = client.id;
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { method: "message", message: messageForLog, id: idForLog } }, json: true } );  // console.log( client.id, "pending", "setState" );
                    client.pending = false;
                    for ( var j = 0; j < global.instances[ namespace ].pendingList.length; j++ ) {
                        client.emit( 'message', messageForLog = global.instances[ namespace ].pendingList[ j ] );
idForLog = client.id;
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { method: "message", message: messageForLog, id: idForLog } }, json: true } );  // console.log( client.id, "pending", global.instances[ namespace ].pendingList[ j ].action );
                    }
...
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { event: "disconnect", id: socket.id } }, json: true } );
        ...
        for ( var i in global.instances[ namespace ].clients ) {
            var client = global.instances[ namespace ].clients[ i ];
            if ( ! client.pending ) {
                client.emit ( 'message', messageForLog = clientMessage );
idForLog = client.id;
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { method: "message", message: messageForLog, id: idForLog } }, json: true } );  // console.log( client.id, "deleteChild" );
            }
        }
        if ( global.instances[ namespace ].pendingList.pending ) {
...
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { event: "error", error: error, id: socket.id } }, json: true } );
    } );
...
function Evaluate( namespace, node, expression ) {
    ...
    return new Promise( function( resolve, reject ) {
        ...
        if ( firstClient ) {
            ...
            firstClient.emit( "message", messageForLog = { node: node, action: "execute", parameters: [ expression ], respond: true, time: global.instances[ namespace ].getTime() } );
idForLog = firstClient.id;
rp.post( "http://localhost:5984/vwf-log", { body: { time: (new Date).toISOString(), service: "server", location: { path: namespace.replace( /\/index\.vwf/, "" ) + "/" }, module: "reflector", facet: "socket", socket: { method: "message", message: messageForLog, id: idForLog } }, json: true } );  // console.log( firstClient.id, "execute", expression );
        } else {
