...
var strategy = new passportLocal( strategyOptions, function( req, username, password, done ) {
  ...
  var user = {
    ...
    instructor:
      false, // set below if authenticated
  };
...
router.get( "/login", function( req, res ) {
  res.render( "login", { title:version.getDerivativeVersion() || "ITDG" } );
} );
...
// Enforce authentication rules. Require a valid login for `/`. For other paths, automatically log
// the user in using a random name. `/login`, `/logout`, and any routes handled before here do not
// require a login and do not automatically log the user in.
...
router.use( function( req, res, next ) {
  ...
  if ( req.isUnauthenticated() ) {
    ...
    if ( req.path === "/" ) {
      ...
      var user = {
        last_name: "user_" + crypto.randomBytes( 32 ).toString( "hex" ),
        first_name: "",
        middle_initial: "",
        instructor: false,
      };
