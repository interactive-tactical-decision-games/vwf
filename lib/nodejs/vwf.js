...
function HandleParsableRequest( request, response ) {
    ...
    var lastURLCharacter = request.url.substr(request.url.length - 1);

    // Hacky way to redirect in the case of a loading link not being terminated by a slash
    if ( ( request.url.indexOf('/load/') !== -1 ) && ( lastURLCharacter != '/' ) ){
        serve.Redirect( request.url + "/", response );
        return true;
    }
    else if ( ( request.url[ request.url.length - 1 ] != "/" ) && ( parsedRequest[ 'private_path' ] == undefined ) && ( url.parse( request.url ).search == undefined || 
